...
class BehaviorPipeline:
    def __init__(self, cfg_path: str, model_override: str = None, show_bbox: bool = False):
        """
        全能行为识别管道 (支持 .onnx 和 .pth，含业务逻辑修正 & 平滑处理 & 性能统计 & 计算量/参数量动态统计)
        """
        print(f"🔧 Loading configuration from: {cfg_path}")
        self.cfg = load_config(cfg_path)
        self.show_bbox = show_bbox  # ✨ 新增参数控制是否画框
        ...
        if model_override:
            ...
            name = task_cfg.get("output_model_name", "repvit_best_distilled_0313.onnx")
            self.model_path = os.path.join(out_dir, name)
            if not os.path.exists(self.model_path):
                self.model_path = "checkpoints/repvit_best_distilled_0316_ab.onnx"
    ...
    def _calc_model_complexity(self):
        ...
        try:
            ...
            self.stats["cls_gflops"] = (macs_cls * 2) / 1e9
            self.stats["cls_params"] = params_cls / 1e6
        ...
        try:
            ...
            if hasattr(self.detector, 'model') and hasattr(self.detector.model, 'model'):
                ...
                self.stats["yolo_gflops"] = (macs_yolo * 2) / 1e9
    ...
    def process_frame(self, frame):
        ...
        if valid_ids:
            ...
            for i, tid in enumerate(valid_ids):
                ...
                # 只更新状态
                self._update_result_status(tid, final_cls_id, final_conf)

        # ✨ 新增：如果启用了 show_bbox，则在画面上画框
        if self.show_bbox:
            for det in detections:
                tid = det["track_id"]
                if tid is None: continue
                ...
                # 从 last_labels 获取当前的颜色，如果没有则用默认绿色
                color = (0, 255, 0)
                label_text = f"ID: {tid}"
                if tid in self.last_labels:
                    text, color = self.last_labels[tid]
                    label_text = f"ID:{tid} {text}"
                
                cv2.rectangle(annotated, (x1, y1), (x2, y2), color, 2)
                cv2.putText(annotated, label_text, (x1, max(0, y1-5)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

        # 统一在画面左上角绘制全局标签
        self._draw_global_labels(annotated, current_ids)
    ...
    def _update_result_status(self, tid, cls_id, conf):
        """仅记录目标的最新状态分类，不再画框"""
        ...
        color = (0, 255, 0) # 默认绿色
        if label.lower() in ["eat", "drink", "vomit", "convulsion"]:
            color = (0, 0, 255) # 特定行为标红

        self.last_labels[tid] = (f"{label} {conf:.2f}", color)

    def _draw_global_labels(self, img, current_ids):
        """将当前活跃目标的状态统一绘制在画面左上角"""
        y_offset = 80  # 预留给FPS信息的空间

        # 绘制标题
        cv2.putText(img, "Live Inference:", (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        y_offset += 35

        active_shown = False
        for tid in sorted(list(current_ids)):
            if tid in self.last_labels:
                text, color = self.last_labels[tid]
                cv2.putText(img, f"[Target {tid}] Behavior: {text}", (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                y_offset += 30
                active_shown = True
...
def run(cfg_path, video_path, model_path=None, save_path=None, show_bbox=False):
    try:
        # 传递 show_bbox 给 pipeline
        pipe = BehaviorPipeline(cfg_path, model_override=model_path, show_bbox=show_bbox)
    except Exception as e:
    ...
    if save_path:
        ...
        final_save_path = os.path.join(save_path, video_name)
        
        writer = cv2.VideoWriter(final_save_path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
        print(f"🎬 Video will be saved to: {final_save_path}")
    else:
        writer = None
    ...
    try:
        while True:
            ...
            info = f"FPS: {sum(fps_history)/len(fps_history):.1f} ({mode}) "
...
if __name__ == "__main__":
    ...
    run(cfg, args.video, args.model, args.save, args.show_bbox)