...
# ==============================================================================
# 🔥 [1. 分層優先 Loss] (Normal vs Abnormal, Convulsion vs Vomit)
# ==============================================================================
class HierarchicalPriorityLoss(nn.Module):
    def __init__(self, normal_idx=0, binary_weight=1.0, fine_weight=1.0, boost_idx=1, boost_weight=2.0):
        ...
        self.boost_idx = boost_idx      # 需加权的类别（如抽搐）
    ...
    def forward(self, logits, targets):
        # --- Binary Loss: (Convulsion+Vomit) vs Normal ---
        binary_targets = (targets != self.normal_idx).float() # 1为异常，0为Normal
        ...
        # --- Fine Loss: Convulsion vs Vomit ---
        mask_abnormal = (targets != self.normal_idx)
        loss_fine = torch.tensor(0.0, device=logits.device)
        
        if mask_abnormal.sum() > 0:
            # 取出类别 1(convulsion) 和 2(vomit) 的 logits
            abnormal_logits = logits[mask_abnormal][:, 1:3]
            # 标签平移: 1 -> 0, 2 -> 1, 以匹配 CrossEntropy
...
class CompositeDistillLoss(nn.Module):
    def __init__(self, alpha=1.0, beta=8.0, gamma=1.0, temperature=4.0):
        ...
        # 🔥 GT Loss 使用增強版的分層 Loss (0:normal, 1:convulsion, 2:vomit)
        self.gt_loss_fn = HierarchicalPriorityLoss(
            normal_idx=0, 
            boost_idx=2, # 设定convulsion为重点关注
            binary_weight=1.5, 
            fine_weight=1.0, 
            boost_weight=1.0
        )
...
def train_distill():
    ...
    # 🔥 更改配置为异常行为类别
    task_cfg['recognizer']['class_names'] = ['normal', 'convulsion', 'vomit'] 
    task_cfg['recognizer']['num_classes'] = 3
    ...
    teacher_ckpt = ROOT_DIR + cfg['distillation'].get('teacher_checkpoint', '/teacher/teacher_best_hierarchical.pth')
    if not os.path.exists(teacher_ckpt):
        teacher_ckpt = ROOT_DIR + "/teacher/teacher_best_hierarchical.pth"
    
    print(f"👨‍🏫 Loading Teacher from: {teacher_ckpt}")
    teacher = SwinTeacher(checkpoint_path=teacher_ckpt, num_classes=3).to(device).eval()
    for p in teacher.parameters(): p.requires_grad = False

    print("👶 Building Student (RepViT-GRU)...")
    student = RepViT_GRU(num_classes=3, hidden_dim=256, pretrained=True).to(device)
    ...
    best_acc = 0.0
    out_dir = "outputs/distilled_final"
    ...
    for epoch in range(epochs):
        ...
        try:
            ...
            print(f"           [Pred Norm] [Pred Conv] [Pred Vomit]")
            print(f"True Norm : {cm[0]}")
            print(f"True Conv : {cm[1]}")
            print(f"True Vomit: {cm[2]}")
            norm_recall = cm[0,0] / (cm[0].sum() + 1e-6)
            conv_recall = cm[1,1] / (cm[1].sum() + 1e-6)
            vomit_recall = cm[2,2] / (cm[2].sum() + 1e-6)
            print(f"Recall -> Norm: {norm_recall:.2f} | Convulsion: {conv_recall:.2f} | Vomit: {vomit_recall:.2f}")
        except: pass

        if acc > best_acc:
            best_acc = acc
            torch.save(student.state_dict(), os.path.join(out_dir, "repvit_best_distilled.pth"))
            torch.save(model_ema.module.state_dict(), os.path.join(out_dir, "repvit_best_distilled_ema.pth"))
